...
struct ClientContext : public boost::enable_shared_from_this<ClientContext>
{
	...
	ClientContext(boost::asio::io_service & io_service):m_socket(io_service), m_recv_buffer_index(0)
	{
	...
	void OnSend(const boost::system::error_code &ec, std::list<std::vector<boost::uint8_t>>::iterator itr)
	{
		...
		{
			global_stream_lock.lock();
			std::cout<<"OnSend error: "<<ec.message()<<std::endl;
			global_stream_lock.unlock();
	...
	void OnRecv(const boost::system::error_code &ec, size_t bytes_transfered)
	{
		if(ec)
		{
			global_stream_lock.lock();
			std::cout<<"OnRecv error: "<<ec.message()<<std::endl;
			global_stream_lock.unlock();
		}
		else
		{
			//increase how many bytes we have saved up
			m_recv_buffer_index += bytes_transfered;

			//debug information
			global_stream_lock.lock();
			std::cout<<"Recv "<<bytes_transfered<<"bytes.\n";
			global_stream_lock.unlock();
...
void OnAccept(const boost::system::error_code &ec, boost::shared_ptr<ClientContext> clnt)
{
	...
	{
		global_stream_lock.lock();
		std::cout<<"OnAccept Error: "<<ec.message()<<"\n";
		global_stream_lock.unlock();
	}
	else
	{
...
int main(void)
{
	...
	//for thread safety issues we use one worker thread
	...
	boost::shared_ptr<ClientContext> client {new ClientContext(*io_svc)};
	try
	{
		boost::asio::ip::tcp::resolver resolver{*io_svc};
		boost::asio::ip::tcp::resolver::query query{"127.0.0.1", boost::lexical_cast<std::string>(4444) };
		...
		acceptor->async_accept(client->m_socket, boost::bind(OnAccept, boost::placeholders::_1, client));

		global_stream_lock.lock();
		std::cout<<"listenning on: "<<endpoint<<std::endl;
		global_stream_lock.unlock();
