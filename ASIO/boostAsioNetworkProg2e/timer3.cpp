...
boost::system::error_code global_ec;
...
void TimerHandler(const boost::system::error_code &ec, boost::shared_ptr<boost::asio::deadline_timer> tmr, boost::shared_ptr<boost::asio::io_service::strand> strand)
{
	...
	{
		global_stream_lock.lock();
		std::cout<<"you see this line every 3 seconds\n";
		global_stream_lock.unlock();
		
		tmr->expires_from_now(boost::posix_time::seconds(3));
		tmr->async_wait(strand->wrap(boost::bind(&TimerHandler, global_ec, tmr, strand)));
	}
}
...
int main()
{
	...
	timer->async_wait(strand->wrap(boost::bind(&TimerHandler, global_ec, timer, strand)));
